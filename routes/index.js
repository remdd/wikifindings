      ...
      if (pwTest) {
        User.register(newUser, req.body.password, function (err, user) {
          if (err) {
            ...
            const link = `https://${req.headers.host}/verify/${user.authToken}`;
            console.log(link);
            try {
              ...
              if (newUser.isScientist) {
                ...
                email.send("scientistReg", process.env.ADMIN_EMAILS, {
                  username: user.username,
                });
              }
              req.flash(
                "success",
                "Welcome! To complete your registration, please click the link in the email you have just been sent."
              );
  ...
  passport.authenticate("local", function (err, user, info) {
    ...
    req.logIn(user, function (err) {
      ...
      User.findByIdAndUpdate(
        user._id,
        { $inc: { loginCount: 1 }, lastLoginDate: Date.now() },
        function (err) {
          if (err) {
            console.log(err);
          }
    ...
    [
      ...
      function (token, user, done) {
        try {
          ...
          req.flash(
            "success",
            "An email with a reset link has been sent to " + user.email + "."
          );
          return res.redirect("back");
        } catch (err) {
          done(err, "done");
  ...
  User.findOne(
    ...
    function (err, user) {
      if (!user) {
        req.flash(
          "error",
          "Error: your reset token is invalid or has expired."
        );
                  ...
                  user.save(function () {
                    req.logIn(user, (err) => {
                      if (err) {
                        ...
                        email.send("pwChangeConfirmation", user.email, {
